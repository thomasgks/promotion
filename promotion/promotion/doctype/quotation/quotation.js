...
frappe.ui.form.on("Quotation", {
  ...
  show_promotion_dialog: function (frm) {
    ...
    frappe.call({
      ...
      callback: function (r) {
        if (r.message) {
          frm.trigger(
            "show_promotion_selection_dialog",
    ...
    let d = new frappe.ui.Dialog({
      ...
      primary_action: function (values) {
        ...
        if (selected_promotions.length > 0) {
          frm.trigger("apply_selected_promotions", selected_promotions);
        } else {
  ...
  // Listen to changes in the coupon_code field for auto-application
