...
frappe.ui.form.on("Quotation", {
	...
	apply_promotion_from_coupon: function (frm) {
		...
		frappe.call({
			method: "promotion.promotion.doctype.promotion.promotion.apply_coupon_code",
			args: {
	...
	remove_coupon_promotion: function (frm) {
		// Show loading
		frappe.show_alert({
		...
		// Remove promotion
		frappe.call({
			method: "promotion.promotion.doctype.promotion.promotion.remove_coupon_promotion",
			args: {
				quotation_name: frm.doc.name,
			},
			callback: function (r) {
	...
	show_promotion_dialog: function (frm) {
		...
		frappe.call({
			method: "promotion.promotion.doctype.promotion.quotation_integration.get_quotation_promotion_summary",
			args: {
				quotation_name: frm.doc.name,
			},
			callback: function (r) {
	...
	apply_selected_promotions: function (frm, selected_promotions) {
		...
		selected_promotions.forEach(function (promotion_name) {
			frappe.call({
				method: "promotion.promotion.doctype.promotion.quotation_integration.apply_promotion_to_quotation",
				args: {
					quotation_name: frm.doc.name,
	...
	remove_promotions: function (frm) {
		frappe.confirm(
			...
			function () {
				frappe.call({
					method: "promotion.promotion.doctype.promotion.quotation_integration.remove_promotion_from_quotation",
					args: {
						quotation_name: frm.doc.name,
						promotion_name: "all",
					},
					...
					callback: function (r) {
						if (r.message) {
							if (r.message.valid) {
								...
								if (r.message.promotion) {
									frappe.msgprint(
										__("Associated Promotion: {0}", [
											r.message.promotion.title,
										])
									);
								}
							} else {
